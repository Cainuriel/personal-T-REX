...
async function deployWithFactory(deployer, tokenOwner, agent) {
  ...
  try {
    ...
    const salt = "ALASTRIA_TREX_V1"; // Salt único para este deployment
    
    const tokenDetails = {
      owner: tokenOwner.address,
      name: "Alastria Security Token",
      symbol: "AST",
      decimals: 18,
      irs: ethers.constants.AddressZero, // Crear nuevo IRS
      ONCHAINID: ethers.constants.AddressZero, // Crear nuevo ONCHAINID
    ...
    const claimDetails = {
      claimTopics: [1, 2], // KYC y Acreditado
      issuers: [],
      issuerClaims: []
    };
    ...
    // Obtener direcciones de contratos desplegados
    const tokenAddress = await trexFactory.getToken(salt);
    console.log(`\n📋 DIRECCIONES DE CONTRATOS DESPLEGADOS:`);
    console.log(`Token: ${tokenAddress}`);
    
    return { trexFactory, tokenAddress };
    
  } catch (error) {
    console.error("❌ Error en despliegue con factory:", error.message);
    throw error;
  }
}
